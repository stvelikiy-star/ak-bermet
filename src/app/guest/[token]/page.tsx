    ...
    <main className="min-h-screen bg-gradient-to-b from-emerald-deep to-emerald-900 px-4 py-8 text-white">
      <div className="mx-auto max-w-xl space-y-5">
        <header className="rounded-2xl border border-gold/30 bg-white/10 p-6 text-center backdrop-blur">
          ...
          <p className="mt-2 text-sm text-white/75">{guest.buildingName} · номер {guest.roomNumber}</p>
          <p className="mt-1 text-xs text-white/55">{guest.categoryName} · QR действует до {new Date(guest.expiresAt).toLocaleDateString("ru-RU")}</p>
        </header>
