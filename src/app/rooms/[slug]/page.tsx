...
export default async function RoomDetailPage({
  ...
  return (
    <main>
              ...
              <div className="rounded-2xl border border-gold/20 bg-milk p-6 shadow-soft">
                ...
                <dl className="divide-y divide-gold/10">
                  {room.facts.map((f) => (
                    ...
                    >
                      <dt className="text-sm text-muted">{t(f.label, locale)}</dt>
        ...
        <Container>
          <div className="mx-auto max-w-3xl rounded-2xl border border-gold/20 bg-milk p-6 shadow-soft sm:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-4 border-t border-gold/15 pt-5 text-sm">
              ...
              >
                {t("Публичная оферта", locale)} <IconArrowRight className="h-4 w-4" />
              </Link>
              <Link
                href="/legal/refund"
                className="inline-flex items-center gap-1.5 font-semibold text-emerald-deep transition-colors hover:text-gold-dark"
              >
                {t("Возврат и отмена", locale)} <IconArrowRight className="h-4 w-4" />
              </Link>
            </div>
          </div>
        </Container>
      ...
      <section className="bg-beige py-14 sm:py-20">
        <Container>
          <h2 className="mb-8 text-center font-display text-2xl font-semibold text-emerald-deep sm:text-3xl">
            {t("Другие варианты размещения", locale)}
          </h2>
          <div className="flex flex-wrap justify-center gap-3">
            {roomDetails
              ...
              .map((r) => (
                ...
                >
                  {t(r.title, locale)} <IconArrowRight className="h-4 w-4" />
                </Link>
              ))}
          </div>
        </Container>
      ...
      <PageCTA
        title={t("Узнать наличие", locale)}
        text={t("Напишите нам в WhatsApp — администратор проверит наличие на ваши даты и количество гостей и подтвердит бронирование.", locale)}
        cta={{ label: t("Узнать наличие", locale), href: waHref }}
      />
    </main>
  );
}
