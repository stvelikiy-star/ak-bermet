...
export default function ManagerQrPanel({ rooms }: { rooms: Room[] }) {
  ...
  useEffect(() => { Promise.all([fetch("/api/manager/guest-qr").then((response) => response.json()), fetch("/api/manager/guest-requests").then((response) => response.json())]).then(([qrBody, requestBody]) => { if (qrBody.ok) setItems(qrBody.items ?? []); if (requestBody.ok) setRequests(requestBody.items ?? []); }).catch(() => setMessage("Не удалось прочитать данные гостевого сервиса.")); }, []);
