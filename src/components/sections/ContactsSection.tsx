...
const CONTACT_CONTENT_KEYS = [
  ...
  "home.contacts.title",
  "home.contacts.cta_whatsapp",
  "home.contacts.cta_availability",
  "home.contacts.map_title",
  "home.contacts.map_cta",
] as const;

export default async function ContactsSection() {
  const locale = await getLocale();
  const published = await loadPublishedSiteContent(locale, CONTACT_CONTENT_KEYS);

  return (
    <section id="contacts" className="bg-cream py-16 sm:py-24">
      <div className="mx-auto max-w-site px-4 sm:px-6 lg:px-8">
        <SectionHeading
          eyebrow={contentValue(published, "home.contacts.eyebrow", t("Свяжитесь с нами", locale))}
          title={contentValue(published, "home.contacts.title", t("Контакты", locale))}
          className="mb-12"
        />
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-gold/15 bg-milk p-6 shadow-soft sm:p-8">
            <ul className="space-y-5 text-sm">
              ...
              <li className="flex items-center gap-3">
                ...
                <a href={`tel:+${SITE.phoneRaw}`} className="text-ink/80 hover:text-emerald-deep">
                  {SITE.phoneDisplay}{" "}
              ...
              <li className="flex items-center gap-3">
                ...
                <a href={`tel:+${SITE.springsPhoneRaw}`} className="text-ink/80 hover:text-emerald-deep">
              ...
              <li className="flex items-center gap-3">
                ...
                <a href={`mailto:${SITE.email}`} className="text-ink/80 hover:text-emerald-deep">
                  {SITE.email}
                </a>
              </li>
            </ul>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              >
                {contentValue(published, "home.contacts.cta_availability", t("Узнать наличие", locale))}
              </a>
            </div>
            ...
            >
              <div className="relative flex flex-1 items-center justify-center p-8">
                ...
                <span className="relative flex items-center gap-2 font-display text-lg font-semibold text-gold-soft">
                  ...
                  {contentValue(published, "home.contacts.map_title", t("Ак-Бермет на карте", locale))}
                </span>
              </div>
              <span className="flex items-center justify-center gap-1.5 bg-milk py-3 text-sm font-semibold text-emerald-deep transition-colors group-hover:text-gold-dark">
                {contentValue(published, "home.contacts.map_cta", t("Показать на карте (2GIS)", locale))}{" "}
