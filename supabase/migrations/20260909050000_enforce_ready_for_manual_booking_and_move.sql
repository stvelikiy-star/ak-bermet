-- AK BERMET — fail-closed operational readiness for booking placement.
--
-- Public availability and durable holds already require a room to be both
-- sellable_status='active' and operational_status='ready'. Manual booking and
-- chessboard room moves must enforce the same authority boundary so rooms in
-- checkout/cleaning/inspection states cannot be sold by an internal workflow.
--
-- This migration preserves the existing RPC signatures, SECURITY DEFINER
-- execution model, search_path hardening and authenticated/service_role ACLs.
...
begin
  ...
  if v_room.operational_status <> 'ready' then
    raise exception 'room_not_ready' using errcode = '22023';
  end if;
...
revoke all on function public.fn_create_manual_booking(uuid,text,text,text,date,date,integer,integer,integer,public.lead_source,numeric,text) from public;
revoke execute on function public.fn_create_manual_booking(uuid,text,text,text,date,date,integer,integer,integer,public.lead_source,numeric,text) from anon;
...
grant execute on function public.fn_create_manual_booking(uuid,text,text,text,date,date,integer,integer,integer,public.lead_source,numeric,text) to service_role;
...
begin
  ...
  if v_target.operational_status <> 'ready' then
    raise exception 'room_not_ready' using errcode = '22023';
  end if;
